...
//
// 断点引擎 - 提供断点规则存储、匹配和用户交互逻辑
//
// *** 架构说明 ***
// 此引擎作为 HttpBreakpointPlugin 的内部实现使用，不直接对外暴露
// 插件架构：
//   1. HttpBreakpointPlugin 在 start() 时注册 EventCallbacks 处理器
//   2. CaptureURLProtocol 通过 EventCallbacks 调用断点检查
//   3. 处理器内部委托给 BreakpointEngine.shared 执行实际逻辑
//
// 事件流：
//   CaptureURLProtocol
//     → EventCallbacks.breakpointCheckRequest()
//     → HttpBreakpointPlugin handler
//     → BreakpointEngine.shared.checkRequestBreakpoint()
//
...
final class BreakpointEngine {
    ...
    /// 断点超时时间（秒）
    var breakpointTimeout: TimeInterval = 30
    ...
    /// 更新断点规则列表
    func updateRules(_ newRules: [BreakpointRule]) {
        rulesLock.lock()
    ...
    /// 检查是否有匹配的响应阶段断点规则
    /// 用于预先判断是否需要拦截响应
    func hasResponseBreakpoint(for request: URLRequest) -> Bool {
    ...
    /// 恢复断点（由 Debug Hub 调用）
    func resumeBreakpoint(_ resume: BreakpointResume) {
        Task {
    ...
    /// 恢复断点（直接调用）
    func resumeBreakpoint(requestId: String, action: BreakpointAction) async {
